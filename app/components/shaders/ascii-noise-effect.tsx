...
interface AsciiNoiseEffectProps {
  ...
  crtChromaticAber?: number;
  crtVignette?: number;
  crtFlickerIntensity?: number;
  crtStaticIntensity?: number;
  crtGlowIntensity?: number;
  crtCornerRounding?: number;
}
...
void main(){
  ...
  vec3 c; float l; float z=animTime;
  vec2 p = uv0;
  for(int i=0;i<3;i++){
...
// Scanlines
vec3 scanline(vec2 uv, float time, vec3 col) {
  float line = sin(uv.y * uScanlineCount + time * 0.5) * 0.5 + 0.5;
  // Reduced darkening for brighter output
  float darkening = mix(1.0, line * 0.8, uScanlineIntensity);

  return col * darkening;
}

// Vignette with rounded corners for CRT look
float crtVignette(vec2 uv, float roundness, float intensity) {
  uv = uv * 2.0 - 1.0;
  // Use intensity to control vignette width: lower = wider, higher = tighter
  float edge = mix(1.5, 0.5, intensity);
  float center = mix(0.8, 0.1, intensity);
  float vig = smoothstep(edge, center, length(uv));
  vec2 absUV = abs(uv);
  float corner = length(max(absUV - vec2(0.95), 0.0));
  float cornerMask = 1.0 - smoothstep(0.0, roundness, corner);
  return vig * cornerMask;
}
...
void main() {
  vec2 curvedUV = curveScreen(vUV, uCurvature);

  // Dark green CRT background color (vintage monitor look)
  if (curvedUV.x < 0.0 || curvedUV.x > 1.0 || curvedUV.y < 0.0 || curvedUV.y > 1.0) {
    fragColor = vec4(0.0, 0.02, 0.01, 1.0);
    return;
  }

  vec3 col = chromaticAberration(uTexture, curvedUV, uChromaticAber);

  if (uGlowIntensity > 0.0) {
    vec3 glow = phosphorGlow(uTexture, curvedUV, uResolution);
    col = mix(col, glow, uGlowIntensity * 0.3);
  }

  col = scanline(curvedUV, uTime, col);

  float flicker = 1.0 + (hash(vec2(uTime * 0.1)) - 0.5) * uFlickerIntensity;
  col *= flicker;

  float staticNoise = hash(curvedUV * uTime * 10.0);
  col += vec3(staticNoise) * uStaticIntensity;

  float vig = crtVignette(curvedUV, uCornerRounding, uCrtVignette);
  col *= vig;

  // Boost overall brightness and vibrancy for CRT effect
  col *= 1.3;
...
int getLetterPattern(int idx) {
  // 5x5 bitmap patterns for "PENSAR" (horizontally flipped for rendering)
  // Character rendering flips x, so we encode patterns mirrored
  // Bit index = x + 5*y, where x-flip means display left→right reads x=4→0
  if (idx == 0) return 32045584; // P
  if (idx == 1) return 33059359; // E
  if (idx == 2) return 18667121; // N
  if (idx == 3) return 33061951; // S: ##### | #.... | ##### | ....# | #####
  if (idx == 4) return 4540401;  // A: ..#.. | .#.#. | #...# | ##### | #...#
  if (idx == 5) return 32045649; // R: ####. | #...# | ####. | #..#. | #...#
  return uTransparentBg == 1 ? 0 : 4096; // empty
}

int pickCharFull(float g){
  int n = uTransparentBg == 1 ? 0 : 4096;
  if (g>0.9535) n=33061407; else if (g>0.9302) n=32045630; else if (g>0.9070) n=33081316; else if (g>0.8837) n=32045617; else if (g>0.8605) n=32032318; else if (g>0.8372) n=15255537; else if (g>0.8140) n=15022414; else if (g>0.7907) n=32575775; else if (g>0.7674) n=16267326; else if (g>0.7442) n=18667121; else if (g>0.7209) n=18732593; else if (g>0.6977) n=32540207; else if (g>0.6744) n=32641183; else if (g>0.6512) n=18415153; else if (g>0.6279) n=16272942; else if (g>0.6047) n=15018318; else if (g>0.5814) n=15022158; else if (g>0.5581) n=18405034; else if (g>0.5349) n=32045584; else if (g>0.5116) n=15255086; else if (g>0.4884) n=33061392; else if (g>0.4651) n=18400814; else if (g>0.4419) n=18444881; else if (g>0.4186) n=16269839; else if (g>0.3953) n=6566222; else if (g>0.3721) n=13177118; else if (g>0.3488) n=14954572; else if (g>0.3256) n=17463428; else if (g>0.3023) n=18157905; else if (g>0.2791) n=18393412; else if (g>0.2558) n=32641156; else if (g>0.2326) n=17318431; else if (g>0.2093) n=15239300; else if (g>0.1860) n=18393220; else if (g>0.1628) n=14749828; else if (g>0.1395) n=12652620; else if (g>0.1163) n=4591748; else if (g>0.0930) n=459200; else if (g>0.0698) n=4329476; else if (g>0.0465) n=131200; else if (g>0.0233) n=(uTransparentBg == 1 ? 0 : 4096); else n=(uTransparentBg == 1 ? 0 : 4096); return n;
}
...
int pickCharMinimal(float g){
  ...
  return uTransparentBg == 1 ? 0 : 4096;
}
...
void main(){
  ...
  vec2 cellPos = floor(fragCoord/cellSize);
  vec2 gridSize = floor(uResolution/cellSize);

  // Determine if we're in center region for text
  vec2 centerCell = gridSize * 0.5;
  float textWidth = 6.0; // "PENSAR" = 6 letters
  float textStartX = centerCell.x - textWidth * 0.5;
  int letterIdx = int(cellPos.x - textStartX);
  bool inTextRegion = abs(cellPos.y - centerCell.y) < 1.0 && letterIdx >= 0 && letterIdx < 6;
  ...
  // Replace with letter pattern if in center region and bright enough
  int n;
  if (inTextRegion && g > 0.4) {
    n = getLetterPattern(letterIdx);
  } else {
    n = pickCharSet(g, uCharset);
  }
...
export const AsciiNoiseEffect = ({
  ...
  brightness = 2,
  contrast = 2,
  seed1 = 6.0806063155950865,
  seed2 = 2.195393449053599,
  hue = 0,
  sat = 0,
  gamma = 0.5,
  vignette = 0.54,
  ...
  crtScanlineCount = 800,
  crtChromaticAber = 0.002,
  crtVignette = 0.2,
  crtFlickerIntensity = 0.02,
  crtStaticIntensity = 0.05,
  crtGlowIntensity = 0.3,
  crtCornerRounding = 0.15,
  className,
  ...
  const init = useCallback((gl: Gl, w: number, h: number) => {
    ...
    const progCrt = makeProgram(gl, vs, fsCrt);
    ...
    const uAscii = {
      ...
      uTransparentBg: gl.getUniformLocation(progAscii, "uTransparentBg")!,
    } as const;
    const uCrt = {
      uResolution: gl.getUniformLocation(progCrt, "uResolution")!,
      uTexture: gl.getUniformLocation(progCrt, "uTexture")!,
      uTime: gl.getUniformLocation(progCrt, "uTime")!,
      uCurvature: gl.getUniformLocation(progCrt, "uCurvature")!,
      uScanlineIntensity: gl.getUniformLocation(progCrt, "uScanlineIntensity")!,
      uScanlineCount: gl.getUniformLocation(progCrt, "uScanlineCount")!,
      uChromaticAber: gl.getUniformLocation(progCrt, "uChromaticAber")!,
      uCrtVignette: gl.getUniformLocation(progCrt, "uCrtVignette")!,
      uFlickerIntensity: gl.getUniformLocation(progCrt, "uFlickerIntensity")!,
      uStaticIntensity: gl.getUniformLocation(progCrt, "uStaticIntensity")!,
      uGlowIntensity: gl.getUniformLocation(progCrt, "uGlowIntensity")!,
      uCornerRounding: gl.getUniformLocation(progCrt, "uCornerRounding")!,
    } as const;
    ...
    const texAscii = gl.createTexture()!;
    gl.bindTexture(gl.TEXTURE_2D, texAscii);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
    ...
    const fbAscii = gl.createFramebuffer()!;
    gl.bindFramebuffer(gl.FRAMEBUFFER, fbAscii);
    gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, texAscii, 0);

    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
    gl.bindTexture(gl.TEXTURE_2D, null);

    return { gl, vao, vbo, progNoise, progAscii, progCrt, uNoise, uAscii, uCrt, texScene, fbScene, texAscii, fbAscii };
  }, []);
  ...
  const render = useCallback(
    (tMs: number) => {
      ...
      const { gl, vao, progNoise, progAscii, progCrt, uNoise, uAscii, uCrt, texScene, fbScene, texAscii, fbAscii } = res;
      if (startRef.current === 0) startRef.current = tMs;
      const t = (tMs - startRef.current) / 1000;
      const w = canvas.width,
        h = canvas.height;

      // Pass 1: Render noise to fbScene
      gl.bindFramebuffer(gl.FRAMEBUFFER, fbScene);
      gl.viewport(0, 0, w, h);
      ...
      // Pass 2: Render ASCII to fbAscii (or screen if CRT disabled)
      gl.bindFramebuffer(gl.FRAMEBUFFER, crtEnabled ? fbAscii : null);
      gl.viewport(0, 0, w, h);
      ...
      gl.uniform1f(uAscii.uSpeed, speed);
      ...
      gl.uniform1i(uAscii.uTransparentBg, transparentBg ? 1 : 0);
      gl.drawArrays(gl.TRIANGLES, 0, 6);

      // Pass 3: Render CRT effect to screen (if enabled)
      if (crtEnabled) {
        gl.bindFramebuffer(gl.FRAMEBUFFER, null);
        gl.viewport(0, 0, w, h);
        ...
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, texAscii);
        gl.uniform1i(uCrt.uTexture, 0);
        gl.uniform2f(uCrt.uResolution, w, h);
        gl.uniform1f(uCrt.uTime, t);
        gl.uniform1f(uCrt.uCurvature, crtCurvature);
        gl.uniform1f(uCrt.uScanlineIntensity, crtScanlineIntensity);
        gl.uniform1f(uCrt.uScanlineCount, crtScanlineCount);
        gl.uniform1f(uCrt.uChromaticAber, crtChromaticAber);
        gl.uniform1f(uCrt.uCrtVignette, crtVignette);
        gl.uniform1f(uCrt.uFlickerIntensity, crtFlickerIntensity);
        gl.uniform1f(uCrt.uStaticIntensity, crtStaticIntensity);
        gl.uniform1f(uCrt.uGlowIntensity, crtGlowIntensity);
        gl.uniform1f(uCrt.uCornerRounding, crtCornerRounding);
        gl.drawArrays(gl.TRIANGLES, 0, 6);
      }
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      ...
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, W, H, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
      gl.bindTexture(gl.TEXTURE_2D, rr.texAscii);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, W, H, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
      gl.bindTexture(gl.TEXTURE_2D, null);
    };
