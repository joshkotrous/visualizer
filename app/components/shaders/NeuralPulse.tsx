...
// Dense Web - More nodes, tighter connections, busier network
const fragmentShader = `
  ...
  vec2 getNodePos(int i, float time, float midSigned) {
    float fi = float(i);
    float angle = hash1(fi * 87.123) * 6.28318;
    float radius = 0.12 + hash1(fi * 543.789) * 0.45;
    vec2 basePos = vec2(cos(angle), sin(angle)) * radius;
    
    float moveSpeed = 0.4 + midSigned * 0.25;
    float moveAmp = 0.04 + abs(midSigned) * 0.02;
    basePos.x += sin(time * moveSpeed + fi * 1.3) * moveAmp;
    basePos.y += cos(time * moveSpeed * 0.9 + fi * 1.9) * moveAmp;
  ...
  void main() {
    ...
    // Dense connections - higher threshold
    for (int i = 0; i < NUM_NODES; i++) {
      for (int j = 0; j < NUM_NODES; j++) {
        ...
        // Tighter connection threshold for denser web
        if (nodeDist < 0.45) {
          float connectionStrength = smoothstep(0.45, 0.1, nodeDist);
          
          float d = lineDist(uv, nodeA, nodeB);
          float line = smoothstep(0.008, 0.002, d) * connectionStrength * 0.35;
          
          // Multiple small pulses
          float pulseSpeed = 2.5 + bassSigned * u_intensity;
          float pulsePos = fract(u_time * pulseSpeed * 0.35 + hash(vec2(float(i), float(j))));
          vec2 pulsePoint = mix(nodeA, nodeB, pulsePos);
          float pulseDist = length(uv - pulsePoint);
          float pulse = smoothstep(0.025, 0.0, pulseDist) * connectionStrength;
          pulse *= 0.4 + u_bass * u_intensity * 0.8;
    ...
    for (int i = 0; i < NUM_NODES; i++) {
      ...
      float nodeSize = baseSize + bassSigned * u_intensity * 0.008;
      
      float core = smoothstep(nodeSize, nodeSize * 0.3, d);
      float glow = smoothstep(nodeSize * 2.5, nodeSize, d) * 0.25;
      
      float flicker = 1.0 + trebleSigned * u_intensity * 0.25 * sin(u_time * 12.0 + float(i) * 2.5);
      
      brightness += (core + glow) * flicker * 0.9;
    }
    
    // Smaller central hub
    float hubDist = length(uv);
    float hubSize = 0.03 + bassSigned * u_intensity * 0.015;
    float hub = smoothstep(hubSize, hubSize * 0.3, hubDist);
    float hubGlow = smoothstep(hubSize * 3.5, hubSize, hubDist) * 0.2;
    brightness += (hub + hubGlow) * 1.1;
    ...
    for (int i = 0; i < NUM_NODES; i++) {
      ...
      if (nodeDist < 0.35) {
        ...
        float line = smoothstep(0.006, 0.001, d) * smoothstep(0.35, 0.15, nodeDist) * 0.25;
        brightness += line;
      }
    }
